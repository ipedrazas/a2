...
func (suite *ExternalTestSuite) TestExternalCheck_ID() {
	...
	suite.Equal("custom-check", check.ID())
}

// TestExternalCheck_Name tests that ExternalCheck returns correct name.
func (suite *ExternalTestSuite) TestExternalCheck_Name() {
	check := &ExternalCheck{
		CheckName: "Custom Check",
	}
	suite.Equal("Custom Check", check.Name())
}
...
func (suite *ExternalTestSuite) TestResultFromJSON_Pass() {
	...
	result, err := check.resultFromJSON(out, nil)
