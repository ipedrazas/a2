...
// TestDependencyCheck_ID tests that DependencyCheck returns correct ID.
func (suite *DepsTestSuite) TestDependencyCheck_ID() {
	check := &DependencyCheck{}
	suite.Equal("deps", check.ID())
}

// TestDependencyCheck_Name tests that DependencyCheck returns correct name.
func (suite *DepsTestSuite) TestDependencyCheck_Name() {
	check := &DependencyCheck{}
	suite.Equal("Vulnerabilities", check.Name())
}

// TestDependencyCheck_Run_NoGovulncheck tests that DependencyCheck handles missing govulncheck.
func (suite *DepsTestSuite) TestDependencyCheck_Run_NoGovulncheck() {
	...
	suite.NotNil(result)
	suite.Equal("deps", result.ID)
	suite.Equal("Vulnerabilities", result.Name)
}

// TestDependencyCheck_Run_WithGovulncheck tests that DependencyCheck runs when govulncheck is available.
func (suite *DepsTestSuite) TestDependencyCheck_Run_WithGovulncheck() {
...
func (suite *DepsTestSuite) TestFormatVulnMessage_Single() {
	message := formatVulnMessage(1, "output")
...
func (suite *DepsTestSuite) TestFormatVulnMessage_Multiple() {
	message := formatVulnMessage(5, "output")
...
func (suite *DepsTestSuite) TestFormatVulnMessage_Zero() {
	message := formatVulnMessage(0, "output")
	suite.Contains(message, "0 vulnerabilities found")
}

// TestDependencyCheck_Run_NoVulnerabilities tests that DependencyCheck returns Pass when no vulnerabilities found.
...
// TestDependencyCheck_Run_ErrorHandling tests that DependencyCheck handles govulncheck errors.
