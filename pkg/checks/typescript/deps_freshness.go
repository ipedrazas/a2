...
// DepsFreshnessCheck detects outdated TypeScript/Node.js dependencies.
type DepsFreshnessCheck struct {
	Config *config.TypeScriptLanguageConfig
}

func (c *DepsFreshnessCheck) ID() string   { return "typescript:deps_freshness" }
func (c *DepsFreshnessCheck) Name() string { return "TypeScript Dependency Freshness" }
...
func (c *DepsFreshnessCheck) Run(path string) (checker.Result, error) {
	...
	outdated, err := countOutdatedTsPackages(result.Stdout)
	if err != nil {
		outdated, err = countOutdatedTsPackages(output)
		if err != nil {
...
// countOutdatedTsPackages parses npm/yarn/pnpm outdated --json output.
func countOutdatedTsPackages(output string) (int, error) {
	if output == "" {
		return 0, nil
	}
