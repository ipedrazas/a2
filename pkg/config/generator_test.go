...
func (s *GeneratorTestSuite) TestToYAML_DefaultOptions() {
	...
	// The generated baseline must round-trip through YAML parsing without error.
	// (Done before shadowing the yaml package name with a local variable below.)
	cfg2 := DefaultConfig()
	assert.NoError(s.T(), yaml.Unmarshal(yamlData, cfg2))

	out := string(yamlData)
	assert.Contains(s.T(), out, "# A2 Configuration")
	assert.Contains(s.T(), out, "# Generated by: a2 init")
	// The default template is now an active annotated baseline, not all-comments.
	assert.Contains(s.T(), out, "files:")
	assert.Contains(s.T(), out, "coverage:")
	assert.Contains(s.T(), out, "threshold: 80")
	assert.Contains(s.T(), out, "execution:")
	assert.Contains(s.T(), out, "parallel: true")
}
