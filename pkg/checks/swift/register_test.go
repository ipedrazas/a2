...
func (s *RegisterTestSuite) TestRegister_CheckOrder() {
	...
	expectedOrders := []int{100, 110, 120, 200, 210, 220, 230}
...
func (s *RegisterTestSuite) TestRegister_AllSwiftLanguage() {
	cfg := config.DefaultConfig()

	checks := Register(cfg)

	for _, check := range checks {
		s.Contains(check.Meta.Languages, checker.LangSwift)
	}
}

func (s *RegisterTestSuite) TestRegister_CoverageThreshold() {
	cfg := config.DefaultConfig()
	cfg.Language.Swift.CoverageThreshold = 90.0

	checks := Register(cfg)

	// Find coverage check
	for _, check := range checks {
		if check.Meta.ID == "swift:coverage" {
